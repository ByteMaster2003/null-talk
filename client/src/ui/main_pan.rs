use crate::{
    data,
    types::{LogLevel, LogMessage, Message, Panels},
};
use chrono::DateTime;
use ratatui::{
    Frame,
...
fn render_main_header(frame: &mut Frame, header_area: Rect, border_color: Color) {
    ...
    let header_text: String = match active_session {
        Some(session) => format!("{:?}: {}", session.name, session.id),
...
fn format_message(message: Message, user_id: String, message_area: Rect) -> ListItem<'static> {
    ...
    let line1 = Line::from(Span::styled(
        format!("{} | {}", msg.username, date_time_string,),
        prompt_style,
    ))
    .alignment(Alignment::Left);

    // Second line message content
    let wrapped_lines = wrap_text_to_width(&msg.content, message_area.width);
